...
#import "YXNDNslookup.h"

const int kYXNDTypeA = 1;
const int kYXNDTypeCname = 5;

@implementation YXNDRecord
...
static NSArray *query_ip(res_state res, const char *host) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        YXNDRecord *record = [[YXNDRecord alloc] init:val ttl:ttl type:t];
...
@interface YXNDNslookup ()

@property (readonly) NSString *domain;
@property (readonly) id<YXNDOutputDelegate> output;
@property (readonly) YXNDNslookupCompleteHandler complete;
@property (readonly) NSString *dnsServer;
@property (atomic) BOOL stopped;

@end

@implementation YXNDNslookup
...
+ (instancetype)start:(NSString *)domain
               output:(id<YXNDOutputDelegate>)output
             complete:(YXNDNslookupCompleteHandler)complete {
    return [YXNDNslookup start:domain server:nil output:output complete:complete];
}
...
+ (instancetype)start:(NSString *)domain
               ...
               output:(id<YXNDOutputDelegate>)output
             complete:(YXNDNslookupCompleteHandler)complete {
    YXNDNslookup *instance = [[YXNDNslookup alloc] init:domain server:dnsServer output:output complete:complete];
