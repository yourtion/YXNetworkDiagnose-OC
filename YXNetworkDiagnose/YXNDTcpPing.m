...
@implementation YXNDTcpPingResult
...
- (YXNDTcpPingResult *)buildResult:(NSInteger)code
    ...
    if (code != 0 && code != kYXNDRequestStoped) {
        return [[YXNDTcpPingResult alloc] init:code max:0 min:0 avg:0 count:1];
    }
    ...
    return [[YXNDTcpPingResult alloc] init:code max:max min:min avg:avg count:count];
}
...
+ (instancetype)start:(NSString *)host
               output:(id<YXNDOutputDelegate>)output
             complete:(YXNDTcpPingCompleteHandler)complete {
    return [YXNDTcpPing start:host port:80 count:3 output:output complete:complete];
}
...
+ (instancetype)start:(NSString *)host
             ...
             complete:(YXNDTcpPingCompleteHandler)complete;
{
    YXNDTcpPing *t = [[YXNDTcpPing alloc] init:host
                                        port:port
                                      output:output
                                    complete:complete
                                       count:count];
