...
const int kYXNDInvalidPingResponse = -22001;

@interface YXNDPingResult ()
...
@implementation YXNDPingResult

- (NSString *)description {
    if (_code == 0 || _code == kYXNDRequestStoped) {
...
enum {
    kYXNDICMPTypeEchoReply = 0,
    kYXNDICMPTypeEchoRequest = 8
};
...
const int kYXNDPacketSize = sizeof(ICMPPacket) + 100;

const int kYXNDPacketBufferSize = 65535;
...
static ICMPPacket *build_packet(uint16_t seq, uint16_t identifier) {
    ICMPPacket *packet = (ICMPPacket *)calloc(kYXNDPacketSize, 1);
    
    packet->type = kYXNDICMPTypeEchoRequest;
    packet->code = 0;
    packet->checksum = 0;
    packet->identifier = OSSwapHostToBigInt16(identifier);
    packet->sequenceNumber = OSSwapHostToBigInt16(seq);
    snprintf((char *)packet->payload, kYXNDPacketSize - sizeof(ICMPPacket), "qiniu ping test %d", (int)seq);
    packet->checksum = in_cksum(packet, kYXNDPacketSize);
    return packet;
}
...
static BOOL isValidResponse(char *buffer, int len, int seq, int identifier) {
    ...
    icmpPtr->type == kYXNDICMPTypeEchoReply &&
    icmpPtr->code == 0 &&
...
@property (nonatomic, strong) id<YXNDOutputDelegate> output;
@property (readonly) YXNDPingCompleteHandler complete;
...
@implementation YXNDPing

- (int)sendPacket:(ICMPPacket *)packet
             sock:(int)sock
           target:(struct sockaddr_in *)addr {
    ssize_t sent = sendto(sock, packet, (size_t)kYXNDPacketSize, 0, (struct sockaddr *)addr, (socklen_t)sizeof(struct sockaddr));
    if (sent < 0) {
        return errno;
    }
    return 0;
...
- (int)ping:(struct sockaddr_in *)addr
 ...
 identifier:(uint16_t)identifier
    ...
    ;
    void *buffer = malloc(kYXNDPacketBufferSize);
    
    ssize_t bytesRead = recvfrom(sock, buffer, kYXNDPacketBufferSize, 0,
...
- (YXNDPingResult *)buildResult:(NSInteger)code
                     durations:(NSTimeInterval *)durations
                         count:(NSInteger)count
                          loss:(NSInteger)loss
                     totalTime:(NSTimeInterval)time {
    if (code != 0 && code != kYXNDRequestStoped) {
        return [[YXNDPingResult alloc] init:code max:0 min:0 avg:0 loss:1 count:1 totalTime:time stddev:0];
    }
    ...
    return [[YXNDPingResult alloc] init:code max:max min:min avg:avg loss:loss count:count totalTime:time stddev:stddev];
}
...
- (void)run {
        ...
        if (host == NULL || host->h_addr == NULL) {
            ...
            if (_complete != nil) {
                dispatch_async(dispatch_get_main_queue(), ^(void) {
                    YXNDPingResult *result = [[YXNDPingResult alloc] init:-1006 max:0 min:0 avg:0 loss:0 count:0 totalTime:0 stddev:0];
                    _complete(result);
                });
            }
            return;
    ...
    if (_complete) {
        NSInteger code = r;
        if (_stopped) {
            code = kYXNDRequestStoped;
        }
        YXNDPingResult *result = [self buildResult:code durations:durations count:index - loss loss:loss totalTime:[[NSDate date] timeIntervalSinceDate:begin] * 1000];
...
+ (instancetype)start:(NSString *)host
               output:(id<YXNDOutputDelegate>)output
             complete:(YXNDPingCompleteHandler)complete {
    return [YXNDPing start:host output:output complete:complete interval:200 count:10];
}
...
+ (instancetype)start:(NSString *)host
             ...
             interval:(NSInteger)interval
                count:(NSInteger)count {
    YXNDPing *ping = [[YXNDPing alloc] init:host output:output complete:complete interval:interval count:count];
