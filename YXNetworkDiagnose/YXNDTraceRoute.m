...
#import "YXNDTraceRoute.h"

@interface YXNDTraceRouteRecord : NSObject
...
@implementation YXNDTraceRouteRecord
...
@implementation YXNDTraceRouteResult
...
@interface YXNDTraceRoute ()
@property (readonly) NSString* host;
@property (nonatomic, strong) id<YXNDOutputDelegate> output;
@property (readonly) YXNDTraceRouteCompleteHandler complete;
...
- (NSInteger)sendAndRecv:(int)sendSock
    ...
    YXNDTraceRouteRecord* record = [[YXNDTraceRouteRecord alloc] init:ttl count:TraceMaxAttempts];
...
- (void)run {
        ...
        if (host == NULL || host->h_addr == NULL) {
            ...
            if (_complete != nil) {
                dispatch_async(dispatch_get_main_queue(), ^(void) {
                    YXNDTraceRouteResult* result = [[YXNDTraceRouteResult alloc] init:-1006];
                    _complete(result);
                });
            }
            return;
    ...
    if (-1 == fcntl(recv_sock, F_SETFL, O_NONBLOCK)) {
        ...
        if (_complete != nil) {
            dispatch_async(dispatch_get_main_queue(), ^(void) {
                YXNDTraceRouteResult* result = [[YXNDTraceRouteResult alloc] init:-1];
                _complete(result);
            });
        }
        close(recv_sock);
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        YXNDTraceRouteResult* result = [[YXNDTraceRouteResult alloc] init:code];
        _complete(result);
    });
}
...
+ (instancetype)start:(NSString*)host
               output:(id<YXNDOutputDelegate>)output
             complete:(YXNDTraceRouteCompleteHandler)complete {
    return [YXNDTraceRoute start:host output:output complete:complete maxTtl:30];
}
...
+ (instancetype)start:(NSString*)host
             ...
             complete:(YXNDTraceRouteCompleteHandler)complete
               maxTtl:(NSInteger)maxTtl {
    YXNDTraceRoute* t = [[YXNDTraceRoute alloc] init:host output:output complete:complete maxTtl:maxTtl];
